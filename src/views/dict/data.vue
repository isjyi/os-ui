  ...
  <BasicLayout>
    <template #wrapper>
      <el-card class="box-card">
        <el-form ref="queryForm" v-permisaction="['system:sysdictdata:query']" :model="queryParams" :inline="true">
  ...
  methods: {
    ...
    getType(TypeId) {
      getType(TypeId).then(response => {
        ...
        this.defaultTypeId = response.data.id
        this.getList()
      })
    },
    /** 查询字典类型列表 */
    ...
    resetQuery() {
      ...
      this.queryParams.type_id = this.defaultTypeId
      this.handleQuery()
    },
    /** 新增按钮操作 */
    handleAdd() {
