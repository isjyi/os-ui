...
import { formatJson, validtate } from '@/utils'

export default {
  name: 'Dict',
  data() {
    const validServerError = (rule, value, callback) => {
      if (this.msg) {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有类型数据项?', '警告', {
        ...
        import('@/vendor/Export2Excel').then(excel => {
          ...
          const filterVal = ['id', 'name', 'type', 'status', 'remark']
